...
def load_dataset(dataset_path):
    ...
    with open(dataset_path, encoding='utf_8') as f:
        ...
        output = []
        for line in tqdm(f):
            # Output      (title,   artist,   genre,   lyrics)
...
def construct_input(formated_data, device, max_input_len=1024):
    ...
    dataset = []
    for t in inputs:
        dataset.append(torch.tensor(t, device=torch.device(device)).unsqueeze(1))
