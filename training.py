...
def init_args():
    ...
    # Model hyperparams
    parser.add_argument("--model_size", type=str, default="gpt2-medium", help="Model size (gpt2, gpt2-medium).")
    ...
    parser.add_argument("--train_data_path", type=str, default="content/lyrics_one_genre_varied.csv", help="Train dataset path.")
...
def prepare_train_data(args, enc, device):
    ...
    print('2')
    train_tensor_data = U.construct_input(formated_dataset, device, max_input_len=1024)
    print('3')
...
def main():
    ...
    special_tokens_dict = {
        "additional_special_tokens": [
            '[s:genre]', '[s:artist]', '[s:title]', '[s:lyrics]',
            '[e:genre]', '[e:artist]', '[e:title]', '[e:lyrics]'
        ]
    }
    ...
    if args.train_model:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_data_loader, desc="Training")):
                ...
                outputs = model(
                    input_ids=tok_ids, attention_mask=att_mask, token_type_ids=tok_type_ids,
